...
import 'person_account_details_page.dart'; // Can reuse the details page if I handle type
...
class SupplierDebtsPage extends StatefulWidget {
  final Hotel hotel;
  const SupplierDebtsPage({super.key, required this.hotel});
...
class _SupplierDebtsPageState extends State<SupplierDebtsPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final accounts = await _repository.getAccounts(widget.hotel.id!, type: 'supplier');
      final summary = await _repository.getOverallSummary(widget.hotel.id!, 'supplier');
      setState(() {
        _accounts = accounts;
        _summary = summary;
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: HotelIdentityTitle(title: "ديون الموردين", hotel: widget.hotel),
        centerTitle: true,
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          final result = await Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => AddPersonOperationPage(hotel: widget.hotel, type: 'supplier')),
          );
          if (result == true) _loadData();
        },
        label: const Text("إضافة مورد أو عملية"),
  ...
  Widget _buildSummaryHeader() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: HotelVisualIdentity.colorForHotel(widget.hotel),
      ...
      child: Row(
        children: [
          _buildSummaryItem("لك عنده", _summary['totalDueToMe'], Colors.greenAccent),
          Container(width: 1, height: 40, color: Colors.white24),
          _buildSummaryItem("عليك له", _summary['totalIOwe'], Colors.orangeAccent),
          Container(width: 1, height: 40, color: Colors.white24),
          _buildSummaryItem("الموردين", _summary['accountCount'].toDouble(), Colors.white, isAmount: false),
        ],
      ),
    );
  }
        ...
        child: Row(
          children: [
            CircleAvatar(
              ...
              child: Icon(Icons.local_shipping_outlined, color: identityColor, size: 20),
            ),
