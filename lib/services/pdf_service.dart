      ...
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          if (t.unwithdrawnLines.isNotEmpty) ...[
            pw.Text("📤 مصروفات خارج إيراد اليوم", style: pw.TextStyle(fontSize: 11, font: boldFont, color: PdfColors.orange700)),
            pw.SizedBox(height: 4),
            pw.TableHelper.fromTextArray(
              headers: ["البند", "المصدر", "طريقة الدفع", "المبلغ"],
              data: t.unwithdrawnLines.map((l) => ["${l.icon} ${l.itemName}", l.label, l.methodLabel ?? "-", currency.format(l.amount)]).toList(),
          ...
          if (t.ownerWithdrawalLines.isNotEmpty) ...[
            ...
            pw.Text("👤 مسحوبات المالك", style: pw.TextStyle(fontSize: 11, font: boldFont, color: PdfColors.amber900)),
            pw.SizedBox(height: 4),
            pw.TableHelper.fromTextArray(
              headers: ["طريقة الدفع", "المبلغ"],
              data: t.ownerWithdrawalLines.map((l) => [l.methodLabel, currency.format(l.amount)]).toList(),
              headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.white, font: boldFont, fontSize: 10),
          ...
          if (t.transferLines.isNotEmpty) ...[
            pw.SizedBox(height: 10),
            pw.Text("🔄 تحويل بين المنشآت", style: pw.TextStyle(fontSize: 11, font: boldFont, color: PdfColors.indigo700)),
            pw.SizedBox(height: 4),
            pw.TableHelper.fromTextArray(
              headers: ["البيان", "الجهة", "المبلغ"],
              data: t.transferLines.map((l) => [l.statement, "${l.isOutgoing ? 'إلى' : 'من'} ${l.counterpartHotelName}", currency.format(l.amount)]).toList(),
              headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.white, font: boldFont, fontSize: 10),
